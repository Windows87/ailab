...
CREATE TABLE IF NOT EXISTS `deeplab`.`author_social_networks` (
  ...
  PRIMARY KEY (`social_network_id`, `author_id`),
  INDEX `fk_social_network_has_author_author1_idx` (`author_id` ASC),
  INDEX `fk_social_network_has_author_social_network_idx` (`social_network_id` ASC),
  CONSTRAINT `fk_social_network_has_author_social_network`
    FOREIGN KEY (`social_network_id`)
    REFERENCES `deeplab`.`social_networks` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_social_network_has_author_author1`
    FOREIGN KEY (`author_id`)
    REFERENCES `deeplab`.`authors` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS `deeplab`.`topic_subtopics` (
  ...
  INDEX `fk_topics_has_subtopics_subtopics1_idx` (`subtopics_id` ASC),
  INDEX `fk_topics_has_subtopics_topics1_idx` (`topics_id` ASC),
